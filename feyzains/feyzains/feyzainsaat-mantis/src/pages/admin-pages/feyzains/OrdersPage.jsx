...
const OrdersPage = () => {
  ...
  // Context
  const { orders, orderCount, fetchOrders, deleteOrder, cancelOrder, updateOrder } = useContext(OrderContext);
  const { sellers, fetchSellers } = useContext(SellerContext);
  const { customers, fetchCustomers } = useContext(CustomerContext);
  const { exchangeRate, fetchExchangeRate } = useContext(PanelContext);
    ...
    try {
      ...
      const data = filteredOrders.map((order) => ({
        ...
        'ÇIKAN ÜRÜN KODLARI': order.order_products
          ?.flatMap((op) => op.extracted_products?.map((ep) => ep.product?.code) || [])
          .filter(Boolean)
        ...
        'ÇIKAN ÜRÜNLER': order.order_products
          ?.flatMap((op) => op.extracted_products?.map((ep) => ep.product?.name) || [])
          .filter(Boolean)
          .slice(0, 3) // sadece ilk 3 tanesini al
          .join(', '),
        ...
        'ÇIKAN KONSİNYELER': order.order_products
          ?.flatMap((op) => op.extracted_products?.map((ep) => ep.product?.consignment) || [])
          .filter(Boolean)
          .slice(0, 3) // sadece ilk 3 tanesini al
          .join(', '),
  ...
  return (
    <Box sx={{ flexGrow: 1, p: 3 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" mb={2} gap={2}>
        ...
        <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
          <FormControl size="small" sx={{ minWidth: 200 }}>
            ...
            <Select value={filterCustomer} label="Müşteri" onChange={(e) => setFilterCustomer(e.target.value)}>
      ...
      {isViewOrderDialogOpen && selectedOrderId && (
        <ViewOrderPage open={isViewOrderDialogOpen} onClose={() => handleViewOrder()} orderId={selectedOrderId} />
      )}
      {selectedOrderForWarehouse && (
        <AssignWarehousePopup open={isAssignWarehouseOpen} onClose={() => handleAssignWarehouse()} order={selectedOrderForWarehouse} />
      )}
      {/* Orders Table */}
              ...
              <TableRow key={order?.id}>
                ...
                <TableCell align="right">
                  ...
                  {(isAdmin || userId === order?.seller?.user?.id) && !(isWarehouse || isWarehouseManager) && (
                    <Tooltip title="İptal">
                      <IconButton onClick={() => handleCancelOrderClick(order)}>
                        <CancelIcon />
                      </IconButton>
                    </Tooltip>
                  )}
                  ...
                  {isAdmin && (
                    <Tooltip title="İrsaliye İndir">
                      <IconButton onClick={() => generateInvoicePDF(order, exchangeRate)}>
                        <PictureAsPdfIcon />
                      </IconButton>
                    </Tooltip>
                  )}
      ...
      <Dialog open={cancelDialogOpen} onClose={() => setCancelDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography mb={2}>Bu siparişi iptal etmek istediğinize emin misiniz?</Typography>
          <TextField
            label="İptal Notu (isteğe bağlı)"
            multiline
            fullWidth
            minRows={3}
            value={cancelNote}
        ...
        <DialogActions>
          ...
          <Button onClick={confirmCancelOrder} color="primary">
            Evet
          </Button>
        </DialogActions>
      </Dialog>
