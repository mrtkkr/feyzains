...
const createUser = async (data) => {
  ...
  const fetchExchangeRate = async () => {
    ...
    try {
      const res = await sendApiRequest({ url: "core/exchange_rate/", method: "GET" });
      if (res.response.status === 200) {
        setExchangeRate(res.data?.rate);
        return res.data;
      } else {
        setError("Döviz kuru alınırken bir hata oluştu.");
        console.error("Failed to fetch exchange rate:", res.response);
      }
    } catch (error) {
  ...
  const updateExchangeRate = async (data) => {
    setError(null);
    try {
      const res = await sendApiRequest({ url: "core/exchange_rate/", method: "PUT", body: data });
      if (res.response.status === 200) {
        setExchangeRate(res.data?.rate);
        return res.data;
      } else {
        setError("Döviz kuru güncellenemedi.");
        console.error("Failed to update exchange rate:", res.response);
      }
    } catch (error) {
      setError("API çağrısı başarısız oldu.");
